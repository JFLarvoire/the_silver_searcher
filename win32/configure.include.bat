...
if defined STINCLUDE if not exist "%STINCLUDE%\dict.h" set "STINCLUDE=" &:# Allow overriding with another alias name, but ignore invalid overrides
if not defined STINCLUDE for /f "delims=" %%d in ('"pushd include & cd & popd"') do SET "STINCLUDE=%%d" &:# Default: Use the current directory
if defined NMINCLUDE if not exist "%NMINCLUDE%\debugm.h" set "NMINCLUDE=" &:# Allow overriding with another alias name, but ignore invalid overrides
if not defined NMINCLUDE for /f "delims=" %%d in ('"pushd include & cd & popd"') do SET "NMINCLUDE=%%d" &:# Default: Use the current directory
